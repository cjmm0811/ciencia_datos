...
```
```{r}
storms[, match("year", colnames(storms)):match("hour", colnames(storms))]
```
...
Para mostrarlo con atajo de teclado empleamos __CNTRL + SHIFT + m__

```{r}
a <- filter(storms, long == -65.9 , year == 1981)
select(a, category, status)
```
...
* dimension.petal: Cuyo resultado sea la división de Petal.Length y Petal.Width
...
# En clase
iris %>% 
  mutate(dimension.sepal = Sepal.Length / Sepal.Width,
         dimension.petal = Petal.Length / Petal.Width) %>% 
  select(Species, dimension.sepal, dimension.petal)
```
...
```{r}
# En clase
rename(mtcars, miles_per_gallon = mpg) %>%  head()
```
...
```{r}
mtcars %>% mutate(GAMA.VEHICULO = 
                    ifelse(hp > 180, "ALTA GAMA", "GAMA ESTÁNDAR")) %>% select(hp, GAMA.VEHICULO)
```

```{r}
mtcars %>% mutate(CATEGORIA = 
                    ifelse(hp %in% 51:147, "ESTÁNDAR", 
