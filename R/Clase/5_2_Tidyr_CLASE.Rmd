...
```{r}
gather(billboard, "week", "position", 4:ncol(billboard))
```

```{r}
dim(billboard)
dim(gather(billboard, "week", "position", 4:ncol(billboard)))
```
...
separate(aux.tidy.billboard, date.entered, into = c("year", "month", "day"), sep = "-", remove = F) %>% head()
```
...
new.billboard <- unite(tidy.billboard, "full_song_name", artist, track, sep = ' - ', remove = F)
```
```{r}
head(new.billboard)
```
