...
if (numero %% 2 == 0){
  ...
  if (numero <= 100){
    print(paste("El número ", numero, " es par y además menor o igual que 100"))
  } else {
    print(paste("El número ", numero, " es par y además mayor que 100"))
  }
...
```{r}
# En clase
edad <- as.numeric(readline("Introduce tu edad "))
class(edad)
```
...
if (edad >= 18){
  ...
  print("NO eres mayor de edad, eres menor")
} 
...
```
```{r}
# Accediendo a posición de índice
...
for (item in 1:length(a)){
  print(paste("Posición ", item, " valor ", a[item]))
}
...
Para saber los dataset que están disponibles llamamos a la función `data()`
...
# [iris[[name]][item] for item in 1:nrow(iris) for name in  colnames(iris)]

for (name in colnames(iris)){
  # Accedemos al nombre
  # print(name)
  print(paste("---> Iterando la columna ", name))
  
  for (item in 1:nrow(iris)){
    
    # iris[["Species"]][1]
    print(iris[[name]][item])
  }
}
```
...
for (name in colnames(data)){
  ...
  # Si el nombre es Petal.Width entramos en el segundo bucle
  if (name == "Petal.Width"){
    
    # Iteramos cada valor de Petal.Width
    for (item in 1:nrow(data)){
      
      # Si el valor es <= que la media de Petal.Width 1.19
      if (data[[name]][item] <= mean(data[[name]])){

        # Le asignamos el valor máximo de Petal.Width 2.5
        data[[name]][item] <- max(data[[name]])
      }
    }
  }
}
...
```{r}
data <- iris
media.Petal.Width <- mean(data$Petal.Width)
max.Petal.Width   <- max(data$Petal.Width)
...
for (name in colnames(data)){
  ...
  if (name == "Petal.Width"){
    ...
    for (item in 1:nrow(data)){
      ...
      # Si el valor es <= que la media de Petal.Width 1.19
      if (data[[name]][item] <= media.Petal.Width){

        # Le asignamos el valor máximo de Petal.Width 2.5
        data[[name]][item] <- max.Petal.Width
      }
    }
  }
}
...
```
```{r}
# Podemos modificar el original iris
...
for (name in colnames(iris)){
  ...
  if (name == "Petal.Width"){
    ...
    for (item in 1:nrow(iris)){
      ...
      # Si el valor es <= que la media de Petal.Width 1.19
      if (iris[[name]][item] <= mean(iris[[name]])){

        # Le asignamos el valor máximo de Petal.Width 2.5
        iris[[name]][item] <- max(iris[[name]])
      }
    }
  }
}
